...
class ViralsampleRequest extends BaseRequest
{
    ...
    {
        if($this->input('facility_id') == 7148 || $this->input('receivedstatus') == 2 || $this->input('submit_type') == 'cancel' || $this->input('justification') == 12) return [];
        ...
        $rules['dob'] = array_merge($rules['dob'], [new BeforeOrEqual($this->input('datecollected'), 'datecollected'), 'after_or_equal:-100years', 'before_or_equal:-1month', ]);
